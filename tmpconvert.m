...
cmd = ['mri_convert ' serverDir '/derivatives/freesurfer/' subject '/mri/brainmask.mgz ' serverDir '/derivatives/freesurfer/' subject '/mri/brainmask.nii.gz']
system(cmd)
...
inPath = sprintf('%s/derivatives/freesurfer/%s/mri/brainmask.nii.gz', serverDir, subject);
refPath = sprintf('%s/rawdata/%s/ses-01/anat/%s_ses-01_T1w.nii.gz', serverDir, subject, subject);
outPath = sprintf('%s/derivatives/freesurfer/%s/mri/registered_brainmask_to_T1w.nii.gz', serverDir, subject);
omatPath = sprintf('%s/derivatives/freesurfer/%s/mri/brainmask_to_T1w.mat', serverDir, subject);
